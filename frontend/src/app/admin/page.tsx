  ...
  return (
    <div>
      <div style={{ marginBottom:24 }}>
        ...
        <p style={{ color:"#5A6888", fontSize:13, margin:"4px 0 0" }}>Plateforme MassPay BF</p>
      </div>
      ...
      <div style={{ display:"flex", gap:14, marginBottom:24, flexWrap:"wrap" }}>
        ...
        <StatCard icon={Activity}      label="Volume total"     value={shortFcfa(stats?.total_volume_fcfa ?? 0)}  sub="cumulé"          color="#4B7BFF" />
        <StatCard icon={TrendingUp}    label="Commissions"      value={shortFcfa(stats?.total_commission_fcfa ?? 0)} sub="revenus plateforme" color="#0DC98A" />
        <StatCard icon={AlertTriangle} label="Batchs complétés" value={stats?.total_batches ?? 0}             sub="tous temps"       color="#9B5CF6" />
      </div>

      <div style={{ background:"#111827", border:"1px solid #1C2840", borderRadius:14, overflow:"hidden" }}>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          ...
          <tbody>
            {tenants.map((t, i) => (
              <tr key={t.id} style={{ borderBottom: i < tenants.length-1 ? "1px solid rgba(28,40,64,.5)" : "none" }}>
                <td style={{ padding:"13px 20px" }}>
                  <div style={{ fontWeight:700, fontSize:13 }}>{t.raison_sociale}</div>
                </td>
                <td style={{ padding:"13px 20px", color:"#98A5C4", fontSize:12 }}>{t.secteur}</td>
                <td style={{ padding:"13px 20px", color:"#E4EAF8", fontSize:13, fontWeight:600 }}>
                  {t.wallet ? shortFcfa(t.wallet.total_debited) : "—"}
                </td>
                <td style={{ padding:"13px 20px", color:"#0DC98A", fontSize:13, fontWeight:700 }}>
